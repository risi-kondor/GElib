...
#ifndef _CnineCtensorPackFunctions
#define _CnineCtensorPackFunctions
...
namespace cnine{
  ...
  */
  ...
  class CtensorPack_Cscalar_prod_expr: public Printable{
  public:
    const CscalarObj& c;
    const CtensorPackObj& x;
    CtensorPack_Cscalar_prod_expr(const CscalarObj& _c, const CtensorPackObj& _x):
      c(_c), x(_x){}
    operator CtensorPackObj() const{
      CtensorPackObj R(x,fill::zero);
      R.add(x,c);
      return R;
    }
    string str(const string indent="") const{
      return CtensorPackObj(*this).str(indent);
    }
  };

  
  CtensorPack_Cscalar_prod_expr operator*(const CscalarObj& c, const CtensorPackObj& x){
    return CtensorPack_Cscalar_prod_expr(c,x); 
  }

  CtensorPack_Cscalar_prod_expr operator*(const CtensorPackObj& x, const CscalarObj& c){
    return CtensorPack_Cscalar_prod_expr(c,x); 
  }

  CtensorPackObj& operator+=(CtensorPackObj& R, const CtensorPack_Cscalar_prod_expr& expr){
    R.add(expr.x,expr.c);
    return R;
  }
