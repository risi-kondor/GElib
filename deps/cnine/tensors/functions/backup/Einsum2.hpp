...
namespace cnine{
  ...
  public:
	...
	nconvolutions+=p;
      ...
      for(int i=0; i<x_summation_indices.size(); i++){
	params.xsdims[i]=x.dims[x_summation_indices[i][0]];
	params.xsstride[i]=x.strides.combine(x_summation_indices[i]);
      }
      for(int i=0; i<y_summation_indices.size(); i++){
	params.ysdims[i]=y.dims[y_summation_indices[i][0]];
	params.ysstride[i]=y.strides.combine(y_summation_indices[i]);
      }
      for(int i=0; i<r_summation_indices.size(); i++){
	params.bdims[i]=r.dims[r_summation_indices[i][0]];
	params.bstride[i]=r.strides.combine(r_summation_indices[i]);
      }
