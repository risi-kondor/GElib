...
#ifndef _SO3bipart4_view
#define _SO3bipart4_view
...
namespace GElib{
  ...
  class SO3bipart4_view: public cnine::Ctensor4_view{
  public:
  ...
  public:
    ...
    SO3bipart4_view(float* _arr, const int _n0, const int _n1, const int _n2, const int _n3, 
      ...
      l2=(n2-1)/2;
      ar=arr+l1*s1+l2*s2;
      arc=arrc+l1*s1+l2*s2;
    }

    SO3bipart4_view(float* _arr,  const Gdims& _dims, const Gstrides& _strides, const int _coffs=1, const int _dev=0):
    ...
    SO3bipart4_view(const Ctensor4_view& x):
      Ctensor4_view(x){
      assert(n1%2==1);
      assert(n2%2==1);
      l1=(n1-1)/2;
