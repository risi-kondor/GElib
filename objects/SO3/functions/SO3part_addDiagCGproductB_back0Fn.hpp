...
#ifndef _SO3part_addDiagCGproductB_back0Fn
#define _SO3part_addDiagCGproductB_back0Fn
...
namespace GElib{
  ...
  class SO3part_addDiagCGproductB_back0Fn{
  public:

    void operator()(const SO3part3_view& _x, const SO3part3_view& _r, const SO3part3_view& _y, const int _offs=0){
	...
	cnine::MultiLoop(B,[&](const int b){
		...
		for(int n=0; n<N; n++){
		  x.inc(m1,n,C(m1+l1,m2+l2)*r(m1+m2,_offs+n)*std::conj(y(m2,n)));
		}
	      }
	    }
	  });
	...
	for(int m1=-l1; m1<=l1; m1++){
	  for(int m2=std::max(-l2,-l-m1); m2<=std::min(l2,l-m1); m2++){
	    ...
	    cnine::Ctensor2_add_otimesc_cu(x,r,y,C(m1+l1,m2+l2),stream);
	  }
	}
