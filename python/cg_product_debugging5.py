...
def make_plot_of_errs():
    ...
    np.random.seed()
    x = G.SO3vec.zeros(1, [1 for i in range(input_l+ 1)])
    y = G.SO3vec.zeros(1, [1 for i in range(input_l+ 1)])
    for i in range(input_l+1):
        xs = (1, 2*i+1, 1, 2)
        x.parts[i] = G.SO3part(torch.from_numpy(np.random.randn(*xs)).float())
        y.parts[i] = G.SO3part(torch.from_numpy(np.random.randn(*xs)).float())
    ...
    plt.tight_layout()
    plt.savefig("image_resid_in_%d_out_%d_sparse_rep_%d.png" % (input_l, maxl, rank))
    # plt.show()
